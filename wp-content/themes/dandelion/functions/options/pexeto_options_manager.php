...
class PexetoOptionsManager{
	...
	public $pexeto_options_url='';
	public $pexeto_functions_url='';
	 ...
	 */
	function __construct($themename, $options_url, $functions_url, $uploads_url, $version, $images_url){
	...
	public function print_text_field($value){
		...
		echo '<input class="option-input" name="'.$value['id'].'" id="'.$value['id'].'" type="'.$value['type'].'" value="'.htmlspecialchars($input_value).'" />';
	...
	public function print_multicheck($value){
		...
		foreach ($value['options'] as $option) {
			...
			echo '<div class="check-holder"><a href="" class="check font-icon'.$class.'" title="'.$option['id'].'" aria-hidden="true"></a><span class="check-value">'.$option['name'].'</span></div>'; 
		} 
		echo '</div>';
	...
	public function print_upload($value){
		...
		$std = isset($value['std'])?$value['std']:"";
		$input_value = esc_attr($this->get_field_value($value['id'], $std));
		echo '<input class="option-input upload" name="'.$value['id'].'" id="'.$value['id'].'" type="text" value="'.htmlspecialchars($input_value).'" />';
		echo '<div id="'.$value['id'].'_button" class="upload-button upload-logo" ><a class="pex-button alignright"><span>Select Image</span></a></div><br/>';
		
		//call the script for this upload button particularly
		echo '<script type="text/javascript">jQuery(document).ready(function($){
			pexetoOptions.loadUploader(jQuery("div#'.$value['id'].'_button"));
		});</script>'; 
	...
	public function print_custom($value){
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<div id="'.$field['id'].'_button" class="upload-button upload-logo" ><a class="pex-button alignright"><span>Select Image</span></a></div><br/>';
					echo '<script type="text/javascript">jQuery(document).ready(function($){
			...
			echo '<input type="hidden" name="'.$field['id'].'s" id="'.$field['id'].'s" value="'.esc_attr($saved_value).'" /></div>';
			if(isset($value['preview']) && $value['preview'] == $field['id']){
				$separator = '|*|';
				$images = explode($separator, $saved_value);
				$resized = array();
				for ( $i=0;$i<count($images)-1;$i++ ) {
					$resized[]= pexeto_get_resized_image($images[$i], 150, 150);
				}
				$resized_string=implode($separator, $resized).$separator;
				echo '<input type="hidden" id="'.$value['id'].'_preview" name="'.$value['id'].'_preview" value="'.esc_attr($resized_string).'" />';
			}
			$field_ids[]=$field['id'];
		...
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				pexetoOptions.setCustomFieldsFunc("'.$value['id'].'", ["'.$idsString.'"], ["'.$namesString.'"], "'.$preview.'", "'.PEXETO_TIMTHUMB_URL.'");
			});
		</script>';
