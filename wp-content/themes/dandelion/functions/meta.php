...
add_action( 'admin_menu', 'create_meta_blog_post_box' );
...
$new_meta_boxes =
	...
	array(
		"title" => "Custom thumbnail height (in pixels)",
		"name" => "thumbHeight",
		"type" => "text",
		"description" => "You can set a custom height to the gallery thumbnails in this field.
		This option is applied when the automatic thumbnail generation option below is enabled (set to ON)"
	),
...
/* ------------------------------------------------------------------------*
 * ADD NEW META BOXES TO THE BLOG POSTS
 * ------------------------------------------------------------------------*/

$new_meta_blog_post_boxes =
	array(
...
if(!function_exists('new_meta_blog_post_boxes')){
	/**
	 * Calls the print method for post meta boxes.
	 */
	function new_meta_blog_post_boxes() {
		global $post, $new_meta_blog_post_boxes;

		foreach ( $new_meta_blog_post_boxes as $meta_box ) {
			print_meta_box( $meta_box, $post );
		}
	}
}
...
if(!function_exists('print_meta_box')){
	...
	<h4>'.$meta_box['title'].'</h4></div>';
		...
		case 'upload':
			...
			echo '<a class="button button-upload" id="button-upload-'.$meta_box['name'].'"><span>Select Image</span></a><br/>';

			//call the script for this upload button particularly
			echo '<script type="text/javascript">jQuery(document).ready(function($){
					pexetoOptions.loadUploader(jQuery("#button-upload-'.$meta_box['name'].'"));
				});</script>';
...
if ( !function_exists( 'create_meta_blog_post_box' ) ) {
	/**
	 * Creates a post meta box.
	 */
	function create_meta_blog_post_box() {
		global $theme_name;
		if ( function_exists( 'add_meta_box' ) ) {
			add_meta_box( 'new-meta-blog-post-boxes', '<div class="icon-small"></div> DANDELION POST SETTINGS', 'new_meta_blog_post_boxes', 'post', 'normal', 'high' );
		}
	}
}
...
if ( !function_exists( 'save_postdata' ) ) {
	...
	function save_postdata( $post_id ) {
		global $post, $new_meta_boxes, $new_meta_post_boxes, $new_meta_blog_post_boxes;
		...
		if(isset($post) && isset($post->post_type)){
			if($post->post_type=='page'){
				...
				$meta_boxes = $new_meta_blog_post_boxes;
			}
		}else{
			return $post_id;
		}
