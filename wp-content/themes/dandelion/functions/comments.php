<?php
/**
 * This file contains some functions needed for the comments functionality.
 */

if ( !function_exists( 'pexeto_comment' ) ) {
	/**
	 * Displays a single comment.
	 */
	function pexeto_comment( $comment, $args, $depth ) {
...
?>
	...
	<div class="alignleft"><?php printf( __( '%1$s' ), get_comment_date('F d, Y') ) ?></div>
		<?php if ( $depth==1 ) {?>
	<div class="reply">
	<div class="reply_icon"></div>
		<?php comment_reply_link( array_merge( $args, array( 'depth' => $depth, 'max_depth' => 2, 'reply_text'=>pex_text( '_reply_text' ) ) ) );
			?></div>
		<?php
		}?></div>
