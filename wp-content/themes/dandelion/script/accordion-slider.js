...
(function($){
	$.fn.accordionSlider=function(options){
		...
		var api, 
			...
			isOldIe = browser.msie && parseInt(browser.version, 10)<=8 ? true : false;
		...
		/**
		 * Scales the slider to a smaller size when the window is resized
		 * to fit the parent container
		 */
		function scaleSlider(){
			var scale = parent.width()/options.holderWidth,
				height;

			if(lastScale!==scale){
				root.css({transform:'scale('+scale+')'});
				height = root[0].getBoundingClientRect().height;
		...
		function getHolders(){
			root.find('.accordion-holder').each(function(i){
				...
				if(isOldIe){
					holder.append('<div class="accordion-shadow" />');
				}
